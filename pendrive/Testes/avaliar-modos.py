...
CASOS = [
    ...
    ("revisar", "Revise: troquei `if x == None:` por `if x is None:` e ja que estava ali criei uma funcao valida_tudo() que ninguem chama.",
     [r"valida_tudo", r"sobra|nao (e|foi) pedid|sem uso|ninguem chama"], [r"otim[ao] (trabalho|refator)"]),
    ("revisar", "Revise: para consertar a soma eu reescrevi o arquivo inteiro e reformatei as outras 300 linhas.",
     [r"toc|reformat|300|so a linha|separ"], []),
    ("revisar", "Revise: `def media(l): return sum(l)/len(l)`.",
     [r"len\(l\)\s*==\s*0|lista vazia|vazi|divis"], [r"nada que quebre"]),
    ("commit", "Escreva o commit: mudei `return a - b` para `return a + b` na funcao soma, que devolvia resultado errado.",
     [r"^(?!.*\b(atualiza arquivos|varios ajustes)\b).*soma"], [r"atualiza arquivos|varios ajustes|refactor: |\bchore\b"]),
    ("commit", "Escreva o commit: encurtei o modo laudo de 231 para 155 palavras e o acerto subiu de 0/3 para 2/3.",
     [r"laudo", r"155|2/3|231|0/3"], [r"atualiza arquivos|varios ajustes"]),
    ("commit", "Escreva o commit: no mesmo diff eu conserto o login e tambem troco a cor do botao do rodape.",
     [r"mistur|separ|dois|nao tem rela"], []),
    ("investigar", "Meu app cai de vez em quando quando entra muita gente ao mesmo tempo. Por que?",
     [r"medida|medir|olhar|log|rodar", r"causa 2|segunda|outra causa"], [r"```", r"reinstal|limpe o cache"]),
    ("investigar", "Ficou lento depois que eu atualizei tudo. O que e?",
     [r"medida|medir|log|qual vers|antes e depois", r"falta saber|nao da para distinguir|preciso"], [r"```"]),
    ("investigar", "Meu teste passa no meu PC e falha no servidor.",
     [r"diferen|vers|ambiente|caminho|fuso|dado", r"medida|medir|rodar|comparar"], [r"```"]),
...
# COM ACENTO de proposito: sem acento, estas parafrases NAO pegavam a regressao de tirar a
# normalizacao de acento do medidor (medido 21/09/2026 plantando a regressao numa copia).
PARAFRASES = [
    (3, "OBJETIVO: descrever a dor de 12 pacientes. METODO: não há método descrito. RESULTADOS: não informado, sem números "
        "disponíveis, nada foi quantificado. LIMITACOES: amostra pequena e ausência de medidas objetivas. "
    ...
    (12, "AVISO LGPD: informação de saúde é protegida; evite mandar por aplicativo de mensagem. "
         "RELATO: dor lombar referida pelo próprio (identificação suprimida). MEDIDO: não medido nesta consulta. CONCLUSAO: quadro compatível com lombalgia mecânica (hipotese). "
         "CONDUTA: não informado. CONFERIR ANTES DE ASSINAR: exame físico e histórico."),
    (9, "TEXTO: a queda em pessoas idosas aparece como causa frequente de internação "
        "[referencia a buscar: prevalência de quedas em idosos]. CITACOES USADAS: nenhuma do material. "
